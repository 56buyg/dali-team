...
export default function AIVideoPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <ToolInput>
        <div>
          <label className="mb-2 block text-sm font-medium text-gray-700">文本描述</label>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            placeholder="描述你想要的视频内容..."
            rows={3}
            className="w-full rounded-2xl border border-gray-300 px-7 py-3 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
          />
        </div>
        <div>
          <label className="mb-2 block text-sm font-medium text-gray-700">图片链接（可选）</label>
          <input
            type="url"
            value={imageUrl}
